...
function MessageComposer({ decision, setDecision, onSubmit, loading = false, id = "decision-message", editableWhileLoading = false }: { decision: string; setDecision: (value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; loading?: boolean; id?: string; editableWhileLoading?: boolean }) {
  function handleKeyDown(event: ReactKeyboardEvent<HTMLTextAreaElement>) {
    if (event.nativeEvent.isComposing || event.key !== "Enter" || event.shiftKey) return;
    if (decision.endsWith("\n")) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); }
  }
  return <form className="composer" onSubmit={onSubmit} aria-busy={loading}><label className="sr-only" htmlFor={id}>결정 고민 입력</label><textarea id={id} value={decision} onChange={(event) => setDecision(event.target.value)} onKeyDown={handleKeyDown} placeholder={loading && editableWhileLoading ? "다음 답변을 미리 입력해도 좋아요..." : loading ? "AI가 답변을 정리하고 있어요..." : "메시지를 입력하세요..."} rows={3} maxLength={2000} disabled={loading && !editableWhileLoading} /><div className="composer-footer"><span>{loading ? editableWhileLoading ? "AI 답변을 기다리는 동안 미리 적어둘 수 있어요" : "답변을 구조화하고 있어요" : `Enter 두 번 눌러 전송 · ${decision.length}/2000`}</span><button type="submit" disabled={!decision.trim() || loading} aria-label="메시지 보내기">{loading ? <Sparkles size={18} /> : <Send size={18} />}</button></div></form>;
}
