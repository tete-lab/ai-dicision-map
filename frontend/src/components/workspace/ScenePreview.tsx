...
export function ScenePreview() {
  ...
  return <main className="world-preview"><header><Link href="/">← AI Decision Map</Link><h1>고민마다 다른 결정의 풍경</h1><p>저장된 고품질 풍경과 실제 평가 데이터를 결합한 결정 아틀라스 시연입니다. 아래 대화·평가값은 가상 예시이며 DB 저장이나 AI 호출은 하지 않습니다.</p><label>가상 고민 선택<select value={theme} onChange={(e) => setTheme(e.target.value as SceneTheme)}>{Object.entries(sceneThemes).map(([id, t]) => <option value={id} key={id}>{t.label}</option>)}</select></label></header><DecisionMapGraph result={result} /></main>;
}
