...
- 분석 결과의 결정 지도에도 같은 컴포넌트를 사용합니다. 질문 제목·선택지의 키워드로 유형 하나를 자동 결정하며, 결과 화면에 테마 변경 버튼은 없습니다. 혼합·미분류 고민은 저울로 표시합니다.
- 모든 고민 포인트의 이름·점수·중요도를 동시에 표시합니다. 라벨과 건물·무게추를 연결선으로 연결하고, 하단에서도 전체 점수를 나란히 비교할 수 있습니다.
- 기준별 건물 높이는 평가 점수에, 저울 무게추 부피는 중요도 × 점수에 비례합니다. 미확인과 0점은 구분합니다. 점수는 성공 확률이 아닙니다.
...
- 도시의 대로·교차로·입면·차량·가로수, 섬의 8층 지형·등고선·이정표, 저울의 금속·유광 조약돌·곡면 접시를 구현했습니다. 환경광 반사와 조명을 사용하지만 컨셉 이미지와 동일한 실사 자산이나 실제 지리 정보는 아닙니다.
- 작은 화면에서는 지도 영역만 좌우 스크롤합니다. 점수 라벨을 과도하게 축소하지 않고 전체 비교 카드도 함께 제공합니다.
